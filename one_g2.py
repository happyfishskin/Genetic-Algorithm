...
# 定義基因個體，這裡每個個體有 10 個變數，表示 10 維的 Sphere function
toolbox.register("attr_float", random.uniform, -100, 100)  # 初始化範圍
toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_float, 30)  # 10個基因
...
def main():
    ...
    for gen in range(200000):
        # 選擇下一代的個體
        offspring = toolbox.select(pop, len(pop))  # 锦标赛选择
