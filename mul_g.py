...
class GeneticAlgorithmOptimizer:
    def __init__(self, population_size=100, cxpb=0.5, mutpb=0.2, ngen=200000):
        ...
        self.toolbox.register("individual", tools.initRepeat, creator.Individual, self.toolbox.attr_float, 30)  # 30維
    ...
    def evolve(self, population):
        ...
        for gen in range(self.ngen):
            ...
            population[:] = elite + offspring  # 保留 elite 與 offspring
            ...
            print(f"世代 {gen}, 最佳適應度: {best_fitness}, 最佳個體: {best_individual}")
        ...
        print(f"全局最佳適應度: {global_best_fitness}")
    ...
    def plot_population(self, ax, population, best_individual):
        ...
        # 計算 Schwefel 函數的 z 值
        z_vals = [schwefel_function(x, y) for x, y in zip(x_vals, y_vals)]

        # 繪製種群點
        ax.scatter(x_vals, y_vals, z_vals, c='r', marker='o', s=100, label='Population')

        # 繪製最佳個體
        best_x = best_individual[0]
        best_y = best_individual[1]
        best_z = schwefel_function(best_x, best_y)
...
if __name__ == "__main__":
    optimizer = GeneticAlgorithmOptimizer(population_size=100, cxpb=0.3, mutpb=0.5, ngen=200000)
